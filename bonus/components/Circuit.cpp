...
nts::Tristate Circuit::compute(std::size_t pin)
{
    ...
    for (size_t i = 0; i < this->_components.size(); i++) {
        if (dynamic_cast<Output *>(this->_components.at(i))) {
...
sf::Vector2f getColByComponent(std::string compo)
{
    static std::vector<sf::Vector2f> positions = {sf::Vector2f(100,-100), sf::Vector2f(600,-100), sf::Vector2f(1100,-100)};

    if (compo == "input" || compo == "clock") {
        positions.at(0).y += 150;
        return positions.at(0);
    }
    if (compo == "output") {
        positions.at(2).y += 150;
        return positions.at(2);
    }
    positions.at(1).y += 150;
    ...
    while (std::getline(file, line)) {
        ...
        if (match.size() > 0) {
            if (parseStatus == "chipsets") {
                ...
                splitToArray(line, tokens, " \t");
                try {
                    if ((tmp = factory.createComponent(tokens.at(0), tokens.at(1), getColByComponent(tokens.at(0))))) {
