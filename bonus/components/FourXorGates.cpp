...
#include "FourXorGates.hpp"

FourXorGates::FourXorGates(const std::string &name, sf::Vector2f pos) : AComponent(name) {
...
FourXorGates::~FourXorGates() {

}

nts::Tristate FourXorGates::compute(std::size_t pin) {
    pin--;
    ...
    if (pin == 2)
        this->_pins[2]->setState(gate::Xor(this->_pins[0]->getState(), this->_pins[1]->getState()));
    if (pin == 3)
        this->_pins[3]->setState(gate::Xor(this->_pins[4]->getState(), this->_pins[5]->getState()));
    if (pin == 9)
        this->_pins[9]->setState(gate::Xor(this->_pins[7]->getState(), this->_pins[8]->getState()));
    if (pin == 10)
        this->_pins[10]->setState(gate::Xor(this->_pins[11]->getState(), this->_pins[12]->getState()));
